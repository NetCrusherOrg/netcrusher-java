...
public class NcUdpBaseTest extends AbstractLinuxTest {
    ...
    public void test() throws Exception {
        ProcessWrapper producer = new ProcessWrapper(Arrays.asList(
            "bash",
            "-o", "pipefail",
            "-c", "openssl rand " + BYTES + " | dd bs=1024 oflag=sync | ncat -4 --nodns --send-only --udp 127.0.0.1 50101"
        ));

        ProcessWrapper consumer = new ProcessWrapper(Arrays.asList(
            "bash",
            "-o", "pipefail",
            "-c", "ncat -4 --nodns --recv-only --udp --idle-timeout 3 --listen 127.0.0.1 50101 | dd bs=1k of=/dev/null"
        ));
